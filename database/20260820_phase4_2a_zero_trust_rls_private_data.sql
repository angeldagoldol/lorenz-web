-- ============================================================================
-- DAGOLDOL PHASE 4.2A — ZERO-TRUST RLS & PRIVATE DATA HARDENING
-- Version: 2026-08-20
...
begin
  ...
  if v_missing is not null then
    raise exception 'Phase 4.2A stopped: required public table(s) missing: %', v_missing;
  end if;

  if to_regclass('storage.objects') is null or to_regclass('storage.buckets') is null then
    raise exception 'Phase 4.2A stopped: Supabase Storage catalog is unavailable.';
  end if;
  ...
  if not exists (select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='is_admin') then
    raise exception 'Phase 4.2A stopped: public.is_admin() is missing.';
  end if;
  ...
  if not exists (select 1 from pg_trigger where tgrelid='public.profiles'::regclass and tgname='profiles_guard_role_client' and not tgisinternal) then
    raise exception 'Phase 4.2A stopped: profiles_guard_role_client trigger is missing.';
  end if;
  ...
  if not exists (select 1 from pg_trigger where tgrelid='public.orders'::regclass and tgname='orders_guard_customer_write' and not tgisinternal) then
    raise exception 'Phase 4.2A stopped: orders_guard_customer_write trigger is missing.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Phase 4.2A stopped: profiles.role is missing.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Phase 4.2A stopped: orders.user_id is missing.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Phase 4.2A stopped: Phase 3 ratings ownership columns are missing.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Phase 4.2A stopped: ratings_order_product_unique is missing.';
  end if;
  ...
  if (select count(*) from storage.buckets where id in ('avatars','brand-logos','payment-proofs','payment-settings','product-images')) <> 5 then
    raise exception 'Phase 4.2A stopped: one or more expected Dagoldol Storage buckets are missing.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Phase 4.2A stopped: payment-proofs must be a private bucket before policy hardening.';
  end if;
end
$$;
...
-- --------------------------------------------------------------------------
-- 16. Storage policy canonicalization. This Supabase project was verified to
--     contain only the five Dagoldol buckets listed below, so replacing all
--     storage.objects policies does not remove another application's bucket
--     contract.
-- --------------------------------------------------------------------------
do $$
declare
...
begin
  if exists (
    select 1 from storage.buckets
    where id not in ('avatars','brand-logos','payment-proofs','payment-settings','product-images')
  ) then
    raise exception 'Phase 4.2A stopped: unexpected Storage bucket discovered; audit before replacing storage.objects policies.';
  end if;
...
-- Public buckets deliver public object URLs through the Storage API itself;
-- do not add a blanket storage.objects SELECT policy that would also expose
-- object metadata/listing. SELECT below is limited to owners/admins because
-- Storage upsert requires SELECT in addition to INSERT/UPDATE.
create policy p42_storage_avatar_owner_read
...
create policy p42_storage_avatar_owner_insert
on storage.objects for insert to authenticated
with check (
  bucket_id='avatars'
  and split_part(name,'/',1)=auth.uid()::text
);
create policy p42_storage_avatar_owner_update
...
create policy p42_storage_avatar_owner_delete
...
create policy p42_storage_payment_proof_owner_admin_read
on storage.objects for select to authenticated
using (
...
create policy p42_storage_payment_proof_owner_insert
on storage.objects for insert to authenticated
with check (
  bucket_id='payment-proofs'
  and split_part(name,'/',1)=auth.uid()::text
);
create policy p42_storage_payment_proof_owner_update
on storage.objects for update to authenticated
using (
  bucket_id='payment-proofs'
  and split_part(name,'/',1)=auth.uid()::text
...
create policy p42_storage_payment_proof_owner_delete
on storage.objects for delete to authenticated
using (
  bucket_id='payment-proofs'
  and split_part(name,'/',1)=auth.uid()::text
...
create policy p42_storage_admin_media_read
on storage.objects for select to authenticated
using (
  bucket_id in ('brand-logos','payment-settings','product-images')
  and public.is_admin()
);
create policy p42_storage_admin_media_insert
on storage.objects for insert to authenticated
with check (
  bucket_id in ('brand-logos','payment-settings','product-images')
  and public.is_admin()
);
create policy p42_storage_admin_media_update
on storage.objects for update to authenticated
using (
  bucket_id in ('brand-logos','payment-settings','product-images')
  and public.is_admin()
...
create policy p42_storage_admin_media_delete
on storage.objects for delete to authenticated
using (
  bucket_id in ('brand-logos','payment-settings','product-images')
  and public.is_admin()
...
-- Keep private/public bucket flags explicit. No object contents are changed.
update storage.buckets set public=false where id='payment-proofs' and public is distinct from false;

-- --------------------------------------------------------------------------
-- 17. Post-migration fail-closed structural assertions.
-- --------------------------------------------------------------------------
do $$
declare
  v_bad text;
begin
  ...
  if v_bad is not null then
    raise exception 'Phase 4.2A stopped: non-canonical public policies remain: %', v_bad;
  end if;
  ...
  if exists (
    ...
    raise exception 'Phase 4.2A stopped: non-canonical storage.objects policy remains.';
  end if;

  if has_function_privilege('anon','public.decrement_stock_for_order(jsonb)','EXECUTE') then
    raise exception 'Phase 4.2A stopped: anon can still execute decrement_stock_for_order(jsonb).';
  end if;
  if has_function_privilege('anon','public.restore_stock_for_order(jsonb)','EXECUTE') then
    raise exception 'Phase 4.2A stopped: anon can still execute restore_stock_for_order(jsonb).';
  end if;

  if has_function_privilege('anon','public.p42_lookup_profile_directory(text)','EXECUTE') then
    raise exception 'Phase 4.2A stopped: anon can execute the authenticated directory RPC.';
  end if;

  if exists (
    select 1 from storage.buckets where id='payment-proofs' and public
  ) then
    raise exception 'Phase 4.2A stopped: payment-proofs is public.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Phase 4.2A stopped: anonymous storage.objects SELECT policy remains.';
  end if;
end
$$;
...
-- ============================================================================
-- PHASE 4.2A KNOWN OPEN P0 (INTENTIONAL, NOT AN ACCEPTANCE PASS)
-- ============================================================================
-- `authenticated` still has EXECUTE on decrement_stock_for_order(jsonb) and
-- restore_stock_for_order(jsonb), and an owned-order INSERT policy remains.
-- This is a temporary compatibility boundary only. Run the Phase 4.2B gate and
-- complete the exact Dagoldol 3.3.5 frontend checkout/cancellation cutover
-- before treating Phase 4.2 as closed.
-- ============================================================================
